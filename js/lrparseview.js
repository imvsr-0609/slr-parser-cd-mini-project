...
function parseInput() {
	...
	var rows =
		...
		'</td><td>' +
		formatAction(state, token, false) +
		'</td></tr>\n';
	var i = 2;
	...
	while (
		...
		if (actionElement.actionType == 's') {
			...
			var removeCount = isElement(EPSILON, rule.development)
				? 0
				: rule.development.length * 2;
			var removedElements = stack.splice(
				stack.length - removeCount,
				removeCount,
			);
		...
		var state = lrTable.states[stateIndex()];
		var token =
			stack.length % 2 == 0 ? stack[stack.length - 1] : tokens[tokenIndex];
		action = state[token];
...
function formatInitialParseView(input, maximumStepCount) {
	var result =
		'<p><b>Judgment to be recognized: </b><input class=\'form-control\' id="input" type="text" size="' +
		input.length +
		'" onkeyup="resize(this, 1);" onchange="parseInput();" value="' +
		input +
		'"></p>';
	result +=
		'<p><input class=\'form-control\' id="maximumStepCount" type="hidden" size="' +
		maximumStepCount.toString().length +
		'" onkeyup="resize(this, 1);" onchange="parseInput();" value="' +
		maximumStepCount +
		'"></p>';
	result +=
		'<input type="button" class="btn btn-success btn-gerar btn-lg" value="Recognize Input"><br><br>';
	result += '<br>';
	result += "<table class='table table-bordered'>";
	result += '<thead>';
	result += '<tr><th colspan="4">Judgment to be recognized:</th></tr>';
	result +=
		'<tr><th>Step</th><th>Stack</th><th>Input Tape</th><th>Action</th></tr>';
	result += '</thead>';
